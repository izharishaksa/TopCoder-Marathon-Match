...
public class RectanglesAndHolesVis {
    ...
    class Drawer extends JFrame {
        ...
        class DrawerPanel extends JPanel {
            ...
            public void paint(Graphics g) {
                ...
                g.drawString(COUNT + "", 150, 20);
                g.drawString((COUNT * COUNT * AREA) + "", 200, 20);
                if (OVERLAPPED) {
                    g.drawString("OVERLAPPED", 350, 20);
                }
            }
        }
        DrawerPanel panel;
    ...
    public long runTest() {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = i + 1; j < N; j++) {
                if (overlap(LX[i], RX[i], LX[j], RX[j]) && overlap(LY[i], RY[i], LY[j], RY[j]) && LX[i] != -2000) {
                    overlap[i] = true;
                    overlap[j] = true;
                    OVERLAPPED = true;
